...
This separate harness supports guarded file-to-folder drags in that disposable root. It classifies all visible filenames in one Jev request, groups the answers by destination, then selects and drags each exact group. Native accessibility selects the rows; a fresh observation rebinds their IDs, and the native worker requires the complete selection to match the planned group before dragging. Every member and destination must remain in the approved folder. Selection or drag failures are never retried; unexpected files stop the plan. Use `--single` only to reproduce the older individual-file baseline. The policy cannot read the expected manifest or file contents. Success requires Jev DONE plus an independent check of all nine destinations, SHA-256 hashes, and no loose or extra files. A stopped run needs inspection and a fresh prepared dataset before another attempt. This is an experimental Finder workflow, not general drag support in `npm start`.
